...
--[[
  Validate if group has reached max size.
]]
local function hasGroupReachedMaxSize(group, prefixKey)
  if group ~= nil then
    local groupId = group['id']
    local maxGroupSize = group['maxSize'] or 0
    if maxGroupSize > 0 then
      -- Check if group is already at max capacity
      local groupCount = getGroupCount(groupId, prefixKey)
      if groupCount >= maxGroupSize then
        -- Return error code for group at max capacity
        return true
      end
    end
  end
...
local function storeJob(eventsKey, jobIdKey, jobId, name, data, opts, timestamp,
  ...
  if group ~= nil then
    priority = group['priority'] or priority
    local groupId = group['id']
    table.insert(optionalValues, "gid")
    table.insert(optionalValues, group['id'])
  end
...
local deduplicationJobId = deduplicateJob(prefixKey, opts['de'],
  jobId, deduplicationKey, eventsKey, maxEvents)
if deduplicationJobId then
  return deduplicationJobId
end
local group = opts['group']
if hasGroupReachedMaxSize(group, prefixKey) then
    return -101
end
-- Store the job.
...
if group ~= nil then
    local groupId = group['id']
    addToGroup(opts['lifo'], prefixKey, groupId, jobId, markerKey, priority,
        isPausedOrMaxed, false)
else
    addJobWithPriority(markerKey, priorityKey, priority, jobId,
        priorityCounterKey, isPausedOrMaxed, nil)
end
-- Emit waiting event
